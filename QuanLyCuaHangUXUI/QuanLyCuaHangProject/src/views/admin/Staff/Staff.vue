<template>
  <div class="staff-page" style="width: 1400px; margin-top: 100px">
      ...
      <tbody>
        <tr v-for="staff in paginatedStaffList" :key="staff.maNv">
          ...
          <td class="action-buttons">
            ...
            <button v-if="staff.tenChucVu.toLowerCase() != 'admin'" @click="openEditModal(staff)" class="edit-btn">Sửa</button>
            <button v-if="staff.tenChucVu.toLowerCase() != 'admin'" @click="toggleDelete(staff.maNv)" class="delete-btn">Ẩn</button>
          </td>
        </tr>
      </tbody>
    </table>
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="isEditMode ? updateStaff() : addStaff()">
          ...
          <div class="form-group">
            ...
            <select v-model="currentStaff.maChucVu">
          ...
          <div class="form-group">
            ...
            <input v-model="currentStaff.tenTaiKhoan" required />
          </div>
          <div v-if="!isEditMode" class="form-group">
            <label>Mật Khẩu:</label>
            <input type="password" v-model="currentStaff.matKhau" required />
          </div>
          <div class="form-group">
            <label>Hình Ảnh:</label>
            <input type="file" @change="handleFileUpload" accept="image/*" />
            <img
              v-if="currentStaff.hinhAnh"
              :src="
                isEditMode && !currentStaff.hinhAnh
                  ? getImageUrl(currentStaff.hinhAnh)
                  : currentStaff.hinhAnh
              "
              alt="Hình ảnh nhân viên"
              class="preview-image"
              @error="handleImageError"
...
export default {
  data() {
    return {
      ...
      imageBaseUrl: getApiUrl + '/HinhAnh/NhanVien/', // URL để truy cập hình ảnh
  ...
  methods: {
      ...
      try {
        const response = await axios.get(`${this.apiBaseUrl}/staff`, {
          params: {
            ...
            gioiTinh: this.gioiTinh.toLowerCase(),
            tinhTrang: this.tinhTrang.toLowerCase(),
          },
        })
        ...
        if (error.code === 'ERR_NETWORK') {
          errorMessage =
            'Không thể kết nối đến backend. Vui lòng kiểm tra xem backend có đang chạy trên https://localhost:7139 không.'
    ...
    async updateStaff() {
      try {
        if (!/^\d{12}$/.test(this.currentStaff.cccd.trim())) {
          Swal.fire('Lỗi!', 'CCCD phải là 12 chữ số.', 'error')
          return
        }
        if (!/^0\d{9,10}$/.test(this.currentStaff.sdt.trim())) {
          Swal.fire('Lỗi!', 'Số điện thoại phải bắt đầu bằng 0 và có 10-11 chữ số.', 'error')
          return
        }
        if (!/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/.test(this.currentStaff.email.trim())) {
          Swal.fire('Lỗi!', 'Email không hợp lệ.', 'error')
          return
        }
        if (!['Đang hoạt động', 'Đã tạm khóa'].includes(this.currentStaff.tinhTrang.trim())) {
          Swal.fire(
            'Lỗi!',
            "Tình trạng chỉ được phép là 'Đang hoạt động' hoặc 'Đã tạm khóa'.",
            'error'
          )
          return
        }
        ...
        const response = await axios.put(
          `${this.apiBaseUrl}/staff/${this.currentStaff.maNv}`,
          formData,
          {
            headers: { 'Content-Type': 'multipart/form-data' },
          }
        ...
        this.fetchStaff()
        // Dùng $set để cập nhật lại URL ảnh
        const updatedStaff = response.data;
        const staffIndex = this.staffList.findIndex(staff => staff.maNv === updatedStaff.maNv);
        if (staffIndex !== -1) {
          updatedStaff.hinhAnh = `${updatedStaff.hinhAnh}?t=${new Date().getTime()}`;  // Thêm tham số thời gian để tránh cache
          this.staffList.splice(staffIndex, 1, updatedStaff);
        }
        this.closeForm()
      } catch (error) {
    ...
    async toggleDelete(id) {
      ...
      try {
        const response = await axios.patch(`${this.apiBaseUrl}/staff/${id}/toggle-delete`)
