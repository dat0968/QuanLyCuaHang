                        ...
                        <div class="variant-selection">
                          <select v-model="selectedVariants[product.maSp]" class="form-select mb-2" @change="updateSelectedVariant(product.maSp, $event.target.value)">
                            <option v-for="variant in product.chitietsanphams" :key="variant.maCtsp" :value="variant.maCtsp">
                              ...
                              ].filter(Boolean).join(' - ') || 'Mặc định' }} ({{ variant.donGia }} VNĐ)
                            </option>
                          </select>
                        </div>
                  ...
                  <div class="menu-grid">
                    <div v-for="combo in filteredCombos" :key="combo.maCombo" class="menu-card">
                      ...
                      <div class="menu-card-content">
                        ...
                        <p>{{ combo.moTa }}</p>
                        <p><strong>Giá: {{ calculateComboPrice(combo) }} VNĐ</strong></p>
    ...
    <div class="order-items">
      <div v-for="(item, index) in orderItems" :key="index" class="order-item">
        ...
        <div class="item-controls">
          ...
          <button class="btn btn-danger btn-sm" @click="removeFromOrder(index)">X</button>
        </div>
      </div>
      ...
      <div class="modal-body bill-modal-body">
        <div class="bill-info">
          ...
          <div class="bill-info-row">
            ...
            <span class="bill-value">{{ bill?.ngayTao ? new Date(bill.ngayTao).toLocaleString('vi-VN') : 'N/A' }}</span>
          </div>
          <div class="bill-info-row">
      ...
      <div v-for="table in tables" :key="table.id" :class="getTableClass(table.tinhTrang)" class="table-card">
        <div class="card-body">
          ...
          <div class="card-actions">
            ...
            <button v-if="table.tinhTrang !== 'Đang sử dụng'" class="btn btn-success btn-sm" @click="openOrderModal(table)">Đặt hàng</button>
            <!-- Nút Sửa: chỉ hiển thị khi trạng thái là "Đang sử dụng" -->
            <button v-if="table.tinhTrang === 'Đang sử dụng'" class="btn btn-warning btn-sm" @click="updateStatus(table, 'Trống')">Sửa</button>
...
<div id="print-bill" style="display: none;">
  <div class="print-bill-content">
    ...
    <p class="print-bill-detail"><strong>Ngày Tạo:</strong> {{ bill?.ngayTao ? new Date(bill.ngayTao).toLocaleString('vi-VN') : 'N/A' }}</p>
    
    <h4 class="print-bill-section">THÔNG TIN KHÁCH HÀNG</h4>
    <p class="print-bill-detail"><strong>Tên Khách Hàng:</strong> {{ bill?.hoTenNguoiNhan || 'Khách tại quầy' }}</p>
    <p class="print-bill-detail"><strong>Số Điện Thoại:</strong> {{ bill?.sdt || '0000000000' }}</p>
    <p class="print-bill-detail"><strong>Địa Chỉ Nhận Hàng:</strong> Tại Quầy - Bàn {{ selectedTable?.id || 'N/A' }}</p>
    ...
    <p class="print-bill-detail"><strong>Hình Thức Thanh Toán:</strong> {{ bill?.hinhThucTt || 'Tại quầy' }}</p>
    <p class="print-bill-detail"><strong>Tổng Tiền Hàng:</strong> {{ totalAmount.toLocaleString('vi-VN') }} ₫</p>
    <p class="print-bill-detail"><strong>Phí Vận Chuyển:</strong> {{ bill?.phiVanChuyen || 0 }} ₫</p>
    <p class="print-bill-detail"><strong>Giảm Giá:</strong> {{ discountAmount.toLocaleString('vi-VN') }} ₫</p>
    <p class="print-bill-total"><strong>Tổng Tiền Thanh Toán:</strong> {{ finalAmount.toLocaleString('vi-VN') }} ₫</p>
